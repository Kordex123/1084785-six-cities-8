...
const groupRoomByCity = (offers: RoomSettings[]) =>
  offers.reduce((result: {[key: string]: RoomSettings[]}, offer: RoomSettings) => {
...
function Favourites({offers}: FavouriteSettingProps):JSX.Element {
  ...
  return (
    <div className="page">
      ...
      { Object.values(roomsByCities).length > 0 ? (
        <main className="page__main page__main--favorites">
          <div className="page__favorites-container container">
            <section className="favorites">
              <h1 className="favorites__title">Saved listing</h1>
              <ul className="favorites__list">
                {Object.entries(roomsByCities).map(([city, roomList]) => (
                  <li className="favorites__locations-items" key={city}>
                    ...
                    <div className="favorites__places">
                      {roomList.map(({ id, title, price, previewUrl }) => (
                        <article className="favorites__card place-card" key={id}>
                          <div className="favorites__image-wrapper place-card__image-wrapper">
                            <a href="#">
                              <img className="place-card__image" src={previewUrl} width="150" height="110" alt="Place image"/>
                            </a>
                          </div>
